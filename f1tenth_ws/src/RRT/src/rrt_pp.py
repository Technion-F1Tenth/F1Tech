...
class PurePursuit(object):
    ...
    """
    def __init__(self, plan, lookahead_distance=0.35, kp=0.3):
        self.plan = plan
    ...
    def pursue(self, curr_x, curr_y, heading):
        ...
        while math.sqrt(math.pow(self.plan[i][0]-curr_x, 2) + math.pow(self.plan[i][1]-curr_y,2)) < look:
            i += 1
            if (i > len(self.plan) - 1):
                i = 0
                look -= 0.1
        #marker = self.create_waypoint_marker(i, nearest_wp=True)
